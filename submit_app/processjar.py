...
from .mfparse import (
    parse_manifest,
    max_of_lower_cytoscape_pkg_versions,
    parse_app_dependencies,
)
...
def process_jar(jar_file, expect_app_name):
    ...
    app_name, app_ver, app_works_with, app_dependencies, has_export_pkg = parser_func(
        manifest
    )
    ...
    if expect_app_name and (not app_name == expect_app_name):
        raise ValueError(
            "has app name as <tt>%s</tt> but must be <tt>%s</tt>"
            % (app_name, expect_app_name)
        )
    ...
    try:
        ...
        (msg,) = e.args
        raise ValueError(
            "has a problem with its manifest for entry <tt>Cytoscape-App-Dependencies</tt>: "
            + msg
        )
...
def _app_dependencies_to_releases(app_dependencies):
    for dependency in app_dependencies:
        ...
        if not release:
            raise ValueError(
                'dependency on "%s" with version "%s": no such release exists'
...
def _get_manifest_file(zip_archive):
    try:
        ...
        raise ValueError(
            "does not have a manifest file located in <tt>%s</tt>" % _MANIFEST_FILE_NAME
        )

    if manifest_info.file_size > _MAX_MANIFEST_FILE_SIZE_B:
        raise ValueError(
            "has a manifest file that's too large; it can be at most %d bytes but is %d bytes"
            % (_MAX_MANIFEST_FILE_SIZE_B, manifest_info.file_size)
        )
    ...
    try:
        ...
        raise ValueError(
            "does not have an accessible manifest file located in <tt>%s</tt>"
            % _MANIFEST_FILE_NAME
        )
...
def _parse_simple_app(manifest):
    app_name, app_version = _get_name_and_version(
        manifest, "Cytoscape-App-Name", "Cytoscape-App-Version"
    )
    ...
    if not app_works_with:
        raise ValueError(
            "does not have <tt>Cytoscape-API-Compatibility</tt> in its manifest"
        )
...
def _parse_osgi_bundle(manifest):
    app_name, app_version = _get_name_and_version(
        manifest, b"Bundle-Name", b"Bundle-Version"
    )
    ...
    if not import_packages:
        raise ValueError(
            "does not import any packages--<tt>Import-Package</tt> is not in its manifest"
        )
    ...
    if max_cy_ver:
        ...
        raise ValueError(
            "does not import Cytoscape packages in <tt>Import-Package</tt>"
        )
    try:
    ...
    if app_dependencies_str:
        try:
            ...
            (msg,) = e.args
            raise ValueError(
                "has a problem with the <tt>Cytoscape-App-Dependencies</tt> entry: "
                + msg
            )
    else:
