...
def _user_accepted(request, pending):
    ...
    if app:
        ...
        pending.delete()
        return HttpResponseRedirect(
            reverse("app_page_edit", args=[app.name]) + "?upload_release=true"
        )
    else:
        return html_response(
            "submit_done.html", {"app_name": pending.fullname}, request
        )
...
def confirm_submission(request, id):
    ...
    return html_response(
        "confirm.html", {"pending": pending, "pom_attrs": pom_attrs}, request
    )
...
def _create_pending(
    ...
    if app:
        ...
        release = get_object_or_none(
            Release, app=app, version=version, platform=platform
        )
        if release and release.active:
            raise ValueError(
                "cannot be accepted because the app %s already has a release with version %s"
                % (app.fullname, version)
            )

    pending = get_object_or_none(
        AppPending, fullname=fullname, version=version, platform=platform
    )
    if pending:
        raise ValueError(
            "cannot be accepted because there is already a pending release with that name, version, and platform"
        )

    pending = AppPending.objects.create(
        submitter=submitter,
        fullname=fullname,
        version=version,
        platform=platform,
...
"""
    send_mail(
        f"Bundle Submitted to ChimeraX Toolshed {host}",
        msg,
        settings.EMAIL_ADDR,
...
def submit_api(request, id):
        ...
        if pom_xml_f and javadocs_jar_f:
            if not error_msg:
                ...
                if len(pom_attrs) != len(PomAttrNames):
                    error_msg = (
                        "pom.xml is not valid; it must have these tags under &lt;project&gt;: "
                        + ", ".join(PomAttrNames)
                    )
                pom_xml_f.close()
            ...
            if not error_msg:
                ...
                pending.javadocs_jar_file.save(
                    basename(javadocs_jar_f.name), javadocs_jar_f
                )
                return HttpResponseRedirect(
                    reverse("confirm-submission", args=[pending.id])
                )
    ...
    return html_response(
        "submit_api.html", {"pending": pending, "error_msg": error_msg}, request
    )


def _send_email_for_accepted_app(
    to_email, from_email, app_fullname, app_name, server_url
):
    subject = "ChimeraX Toolshed - {app_fullname} Has Been Approved".format(
        app_fullname=app_fullname
    )
    app_url = reverse("app_page", args=[app_name])
    msg = """Your app has been approved! Here is your app page:
...
- ChimeraX Team
""".format(
        app_url=app_url, author_email=to_email, server_url=server_url
    )
...
def _pending_app_accept(pending, request):
    ...
    _send_email_for_accepted_app(
        pending.submitter.email,
...
def _forward_plugins_xml(request_post):
    try:
        ...
        return HttpResponse(
            "Unable to retrieve: %s" % PluginXmlUrl,
...
def _update_app_page(request_post):
    ...
    if author_count:
        ...
        for i in range(author_count):
            ...
            author_order = OrderedAuthor.objects.create(
                app=app, author=author, author_order=i
            )
