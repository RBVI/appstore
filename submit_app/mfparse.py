...
def parse_manifest(manifest_lines):
    ...
    for line in manifest_lines:
        ...
        if line.startswith(b" "):  # is this line a continuation?
            if not vals:
                continue  # if we haven't read any entries, ignore this continuation
...
def _index_of_char(string, c):
    ...
    while i < strlen:
        if string[i] == c:
            ...
            while i < strlen and not _is_dblquote(
                string, i
...
def parse_app_dependencies(s):
    for dependency in _split_by_pkg(s):
        ...
        if not app_version:
            raise ValueError(
                'app %s has an invalid version: "%s"' % (app_name, app_version_str)
            )
