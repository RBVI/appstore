    ...
    if satisfied:
        ...
        print(
            ...
            % (
                ...
                versionTupleToString(expected_version),
            )
        )
        if pipPkgName:
            pipInstallCmd(pipPkgName)
...
def main():
    ...
    if testPackageAndVersion("PIL", (5, 1, 0), getPILVersion, "Pillow==5.1.0"):
        ...
        testPILImageSupport(
            "PNG", "http://upload.wikimedia.org/wikipedia/commons/8/89/PNG-Gradient.png"
        )
        testPILImageSupport(
            "GIF",
            "http://upload.wikimedia.org/wikipedia/commons/a/a0/Sunflower_as_gif_websafe.gif",
        )
    if testPackageAndVersion("django", (1, 8, 0), getDjangoVersion, "Django==1.8.19"):
        testPackageAndVersion(
            "social_auth", (0, 7), getSocialAuthVersion, "django-social-auth==0.7.23"
        )
    testMaven()
    testUnzip()
