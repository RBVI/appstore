...
class Release(models.Model):
    ...
    platform = models.CharField(max_length=15, blank=True)
    ...
    dependencies = models.ManyToManyField(
        "self", related_name="dependents", symmetrical=False, blank=True
    )

    def __lt__(self, other):
        return (self.app, self.pip_version, self.platform) < (
            other.app,
            other.pip_version,
            other.platform,
        )
...
def javadocs_path(release_api, filename):
    return pathjoin(
        release_api.release.app.name, "releases", release_api.release.version, filename
    )
...
class ReleaseAPI(models.Model):
    ...
    def extract_javadocs_jar(self):
        ...
        subprocess.call(
            ["unzip", file.path, "-d", dirpath], stdout=nullfile, stderr=nullfile
        )
        nullfile.close()
