...
def dom_to_std_obj(dom_obj):
    ...
    result["description"] = (
        dom_obj.getElementsByTagName("description")[0].childNodes[0].nodeValue
    )
    result["version"] = (
        dom_obj.getElementsByTagName("pluginVersion")[0].childNodes[0].nodeValue
    )
    result["release-date"] = (
        dom_obj.getElementsByTagName("release_date")[0].childNodes[0].nodeValue
    )
...
def gather_plugins_by_name_and_version(plugins):
    ...
    for plugin in plugins:
        ...
        version = (
            plugin.getElementsByTagName("pluginVersion")[0].childNodes[0].nodeValue
        )
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for fullname in args:
            ...
            print("%s:" % plugin_name, end="", flush=True)
