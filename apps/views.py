...
class _NavPanelConfig:
    ...
    tag_cloud_delta_font_size_em = (
        tag_cloud_max_font_size_em - tag_cloud_min_font_size_em
    )
...
def _nav_panel_context(request):
    ...
    for tag in all_tags:
        try:
            ...
            rel_count = 1
        font_size_em = (
            rel_count * _NavPanelConfig.tag_cloud_delta_font_size_em
            + _NavPanelConfig.tag_cloud_min_font_size_em
        )
        tag.font_size_em = "%.2f" % font_size_em

    result = {"all_tags": all_tags, "top_tags": top_tags, "not_top_tags": not_top_tags}
...
def apps_default(request):
    latest_apps = App.objects.filter(active=True).order_by("-latest_release_date")[
        : _DefaultConfig.num_of_top_apps
    ]
    downloaded_apps = (
        App.objects.filter(active=True)
        .order_by("downloads")
        .reverse()[: _DefaultConfig.num_of_top_apps]
    )
    ...
    return html_response(
        "apps_default.html", c, request, processors=(_nav_panel_context,)
    )
...
def apps_with_tag(request, tag_name):
    ...
    return html_response(
        "apps_with_tag.html", c, request, processors=(_nav_panel_context,)
    )
...
def apps_with_author(request, author_name):
    ...
    c = {
        ...
        "go_back_to_title": _unescape_and_unquote(
            request.COOKIES.get("go_back_to_title")
        ),
        "go_back_to_url": _unescape_and_unquote(request.COOKIES.get("go_back_to_url")),
    }
    return html_response(
        "apps_with_author.html", c, request, processors=(_nav_panel_context,)
    )
...
def _app_rate(app, user, post):
    ...
    try:
        ...
        raise ValueError(
            'rating is "%s" but must be an integer between 0 and 5' % rating_n
        )
    app.votes += 1
    app.stars += rating_n
    app.save()
...
class _LatestReleases:
    ...
    def get_prereleases(self):
        if self.releases[0] is None or (
            self.prereleases[0] is not None and self.prereleases[0] > self.releases[0]
        ):
    ...
    if request.method == "POST":
        ...
        if not action in _AppActions:
            return HttpResponseBadRequest(
                escape(
                    'action "%s" invalid--must be: %s'
                    % (action, ", ".join(_AppActions))
                )
            )
        try:
...
def _upload_icon(app, request):
    ...
    if f.size > _AppPageEditConfig.max_img_size_b:
        raise ValueError(
            ...
            % (f.size, _AppPageEditConfig.max_img_size_b)
        )
...
def _upload_screenshot(app, request):
    ...
    thumbnail_f = scale_img(
        screenshot_f, screenshot_f.name, _AppPageEditConfig.thumbnail_height_px, "h"
    )
...
def _save_authors(app, request):
    ...
    for name, institution, author_order in authors:
        ...
        ordered_author = OrderedAuthor.objects.create(
            app=app, author=author, author_order=author_order
        )
...
_AppEditActions = {
    ...
    "save_cy_2x_plugin_release_date": _mk_basic_field_saver(
        "cy_2x_plugin_release_date", func=_parse_iso_date
    ),
    ...
    "save_license_confirm": _mk_basic_field_saver(
        "license_confirm", func=lambda s: s.lower() == "true"
    ),
