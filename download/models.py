...
class Download(Model):
    ...
    def __unicode__(self):
        return (
            unicode(self.release)
            + " "
            + unicode(self.when)
            + " "
            + ipaddr_long_to_str(self.ip4addr)
        )


class ReleaseDownloadsByDate(Model):
    release = ForeignKey(
        Release, SET_NULL, null=True
    )  # null release has total count across a given day
    when = DateField()
...
class GeoLoc(Model):
    country = CharField(
        max_length=2
    )  # when region & city are empty, country contains the total
...
class AppDownloadsByGeoLoc(Model):
    app = ForeignKey(
        App, SET_NULL, null=True
    )  # null app has total count across a given geoloc
    ...
    def __unicode__(self):
        return (
            unicode(self.app) + " " + unicode(self.geoloc) + ": " + unicode(self.count)
        )
