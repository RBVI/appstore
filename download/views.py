...
def _all_geography_downloads(app):
    ...
    response = [
        [dl.geoloc.country, dl.geoloc.region, dl.geoloc.city, dl.count] for dl in dls
    ]
...
def _world_downloads(app):
    countries = AppDownloadsByGeoLoc.objects.filter(
        app=app, geoloc__region="", geoloc__city=""
    )
...
def _country_downloads(app, country_code):
    cities = AppDownloadsByGeoLoc.objects.filter(
        app=app, geoloc__country=country_code, geoloc__city__gt=""
    )
...
def app_stats(request, app_name):
    ...
    app_downloads_by_country = AppDownloadsByGeoLoc.objects.filter(
        app=app, geoloc__region="", geoloc__city=""
    )
    releases = app.release_set.all()
    release_downloads_by_date = [
        dl
        for release in releases
        for dl in ReleaseDownloadsByDate.objects.filter(release=release)
    ]
    c = {
        "app": app,
