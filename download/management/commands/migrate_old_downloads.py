...
def increment_count(klass, **args):
    print(klass.__name__, args, ';', end='', flush=True)
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for id, release_id, created, ip4addr in cur.fetchall():
            ...
            print('{0:5} {1:30} {2:15} {3:10} {4:15}'.format(id, app.fullname, release.version, str(when), ipaddr_long_to_str(ip4addr)), end='', flush=True)
